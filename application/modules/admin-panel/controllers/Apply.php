...
class Apply extends Admin_controller  {
	...
	private $table = 'apply';
	protected $redirect = 'apply';
	protected $title = 'Job Apply';
    ...
    {
        check_ajax();
        $this->load->model('Apply_model', 'data');
        ...
        foreach($fetch_data as $row)
        {
            ...
            $sub_array[] = $sr;
            $sub_array[] = $row->name;
            $sub_array[] = $row->phone ? $row->phone : 'NA';
            $sub_array[] = $row->email;
            ...
            $action .= '<a class="dropdown-item" onclick="script.viewApply('.e_id($row->id).');" href="javascript:;"><i class="fa fa-eye"></i> View</a>';
